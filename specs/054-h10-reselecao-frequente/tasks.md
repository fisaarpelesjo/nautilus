...
- [ ] T001 [US1] Teste em `tests/test_pairs_trading.py`: `run_pairs_scan(reselecionar_a_cada=None)` reproduz byte a byte o resultado já publicado (regressão do default)
- [ ] T002 [P] [US1] Teste em `tests/test_pairs_trading.py`: `run_pairs_scan(reselecionar_a_cada=120)` chama `selecionar_pares` mais vezes que com `reselecionar_a_cada=None` (spy sobre `selecionar_pares`), confirmando que o parâmetro desacopla de fato

### Implementation

- [ ] T003 [US1] Adicionar `reselecionar_a_cada: Optional[int] = None` a `run_pairs_scan` (`backtesting/pairs_trading.py`) — `None` preserva `= p.formacao` (depende de T001-T002)
- [ ] T004 [US1] Criar `cmd_pairs_reselecao()` em `main.py`: chama `run_pairs_scan(pares=UNIVERSO_AMPLO_HISTORICO_COMPLETO, reselecionar_a_cada=120)`, imprime treino/validação/veredito e os dois números já publicados (6 trades, specs 039/052) lado a lado; registrar `"pairs_reselecao": cmd_pairs_reselecao` em `COMMANDS`; exportar via `export_report("pairs_reselecao", ...)` (depende de T003)
