...
- [ ] T001 [P] [US1] Teste em `tests/test_onchain.py`: resposta mockada com `status: "ok"` e `values` preenchido — `fetch_onchain_series` retorna `DataFrame` com índice `DatetimeIndex` crescente sem duplicatas e coluna `value` com os valores corretos
- [ ] T002 [P] [US1] Teste: a chamada a `requests.get` (mockada) inclui `sampled=false` e o `timespan` passado como parâmetro — confirma D1 (research.md), sem o qual a API poderia subamostrar o período
- [ ] T003 [P] [US2] Teste: `requests.get` mockado levanta exceção de rede (`ConnectionError`/`Timeout`) — `fetch_onchain_series` propaga como exceção explícita, nunca série vazia silenciosa
- [ ] T004 [P] [US2] Teste: resposta mockada com status HTTP não-200 — `fetch_onchain_series` levanta exceção
- [ ] T005 [P] [US2] Teste: resposta mockada com HTTP 200 mas corpo `{"status": "error", ...}` (nome de métrica inválido) — `fetch_onchain_series` levanta exceção citando o status retornado
- [ ] T006 [P] [US3] Teste: resposta mockada com `status: "ok"` e `values: []` — `fetch_onchain_series` retorna `DataFrame` vazio, **sem** levantar exceção (distingue ausência real de dado de falha, FR-004)

### Implementation for User Story 1 + 2 + 3

- [ ] T007 [US1][US2][US3] Implementar `fetch_onchain_series(metric: str, timespan: str = "3years") -> pd.DataFrame` em `data/onchain.py`: monta a URL (`https://api.blockchain.info/charts/{metric}?timespan={timespan}&format=json&sampled=false`), chama `requests.get(..., timeout=15)`, levanta exceção em falha de rede/HTTP não-200/`status != "ok"`, parseia `values` (`x` unix seconds → `DatetimeIndex` UTC, `y` → coluna `value`), ordena e remove duplicatas de índice (depende de T001-T006)
...
- [ ] T008 Validar manualmente o passo 2 do `quickstart.md` (`fetch_onchain_series("n-unique-addresses", timespan="1years")`) contra a API real — confirma ~365 pontos, sem exceção
- [ ] T009 Validar manualmente o passo 3 do `quickstart.md` (métrica inexistente) contra a API real — confirma exceção
- [ ] T010 Rodar a suite completa (`pytest -q`) para confirmar ausência de regressão em `data/fetcher.py` e `data/sources/` (intocados)
